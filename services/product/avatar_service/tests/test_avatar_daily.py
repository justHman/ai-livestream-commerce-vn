...
from avatar.observability import scoped
from avatar.observability.logging.config import validate_config
...
from avatar.observability.logging.platform_collector import PlatformCollector
...
def render(**extra: object) -> str:
    ...
    logger = logging.getLogger("avatar.observability.test.daily_render")
...
def test_setup_attaches_console_and_daily_handlers() -> None:
    with removable_temp_dir() as runtime_root:
        ...
        try:
            owned = [
                h for h in logger.handlers if getattr(h, "_avatar_observability_handler", False)
            ]
            assert len(owned) == 2
...
def test_setup_is_idempotent_and_closes_owned_handlers() -> None:
    with removable_temp_dir() as runtime_root:
        logger = setup_logging(validate_config(runtime_root=runtime_root))
        owned = [h for h in logger.handlers if getattr(h, "_avatar_observability_handler", False)]
        assert len(owned) == 2
        setup_logging(validate_config(runtime_root=runtime_root))
        assert (
            len([h for h in logger.handlers if getattr(h, "_avatar_observability_handler", False)])
            == 2
        )
        reset_logging()
