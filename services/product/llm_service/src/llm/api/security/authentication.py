"""Constant-time bearer authentication for the LLM service."""
...
def authenticate_bearer(
    ...
    Returns "service" for the service token and "admin" for the admin token.
    """
    if not config.auth_enabled:
        # Auth disabled = trusted internal runtime (default dev path).
        # Scopes then validate as a service subject.
        return "service"
...
def require_auth(
    ...
    """Compatibility dependency: authenticate and return the subject.

    Reads the security config from request app state at call time.
    """
