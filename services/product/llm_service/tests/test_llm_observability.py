...
from llm.observability.logging.config import REDACTION_FIELD, REDACTION_MARKER
...
def render_with_downstream_handler(extra: dict[str, object]) -> tuple[str, logging.LogRecord]:
    ...
    safe_handler.setFormatter(ContextFormatter(service="llm"))
    safe_handler.addFilter(StructuredFieldsFilter())
    downstream_handler = CaptureHandler()
    logger = logging.getLogger("llm.observability.test.downstream")
    old_handlers, old_propagate = logger.handlers[:], logger.propagate
    ...
    with removable_temp_dir() as runtime_root:
        ...
        def owned() -> list[logging.Handler]:
            return [
                ...
                if getattr(item, "_llm_observability_handler", False)
            ]
