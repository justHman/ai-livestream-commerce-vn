...
from llm.observability import scoped
from llm.observability.logging.config import validate_config
...
from llm.observability.logging.platform_collector import PlatformCollector
...
def render(**extra: object) -> str:
    ...
    logger = logging.getLogger("llm.observability.test.daily_render")
...
def test_setup_attaches_console_and_daily_handlers() -> None:
    with removable_temp_dir() as runtime_root:
        ...
        try:
            owned = [
                h for h in logger.handlers if getattr(h, "_llm_observability_handler", False)
            ]
            assert len(owned) == 2
...
def test_setup_is_idempotent_and_closes_owned_handlers() -> None:
    with removable_temp_dir() as runtime_root:
        logger = setup_logging(validate_config(runtime_root=runtime_root))
        owned = [h for h in logger.handlers if getattr(h, "_llm_observability_handler", False)]
        assert len(owned) == 2
        setup_logging(validate_config(runtime_root=runtime_root))
        assert (
            len([h for h in logger.handlers if getattr(h, "_llm_observability_handler", False)])
            == 2
        )
        reset_logging()
