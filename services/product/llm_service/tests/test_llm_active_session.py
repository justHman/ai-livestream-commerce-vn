...
from llm.observability.logging.active_session_handler import (
    ACTIVE,
    CLOSED,
    INACTIVE,
    ActiveSessionHandler,
)
from llm.observability.logging.platform_collector import (
    PlatformCollector,
...
def test_product_services_are_rejected_for_platform_group() -> None:
    with removable_temp_dir() as active_root:
        with pytest.raises(ValueError, match="Unknown platform service"):
            ActiveSessionHandler(service="llm", group="platform", active_root=active_root)
...
def test_cli_help_succeeds() -> None:
    module = "llm.observability.logging.platform_collector"
...
def test_run_command_collects_stdout_and_stderr_concurrently() -> None:
    from llm.observability.logging.platform_collector import run_command
...
def test_platform_collector_writes_only_normalized_events_to_exact_log() -> None:
    with removable_temp_dir() as active_root:
        ...
        try:
            ...
            assert not active_root.joinpath("platform", "llm.log").exists()
        finally:
            collector.close()
