...
Threading contract:
  - ONE producer thread iterates ``stream_chunks`` and puts typed events
    (``DeltaEvent`` / ``EofEvent`` / ``ErrorEvent``) into a bounded FIFO.
...
class LLMStreamController:
    ...
    def start(self) -> None:
        ...
        self._generator = self._llm.stream_chunks(
            self._req,
    ...
    def get(self, timeout: Optional[float]) -> Optional[StreamEvent]:
        """Return the next event, or None after ``timeout`` seconds.

        ``timeout=None`` waits for the next event indefinitely — the caller
        uses it when the chunker buffer has no live deadline (empty or below
        the min-chars floor), so TTFT never fakes a deadline.
        """
        try:
