...
def _product(**kwargs: object) -> ProductEntityIn:
    """One minimal product input (8.12: entity API model, no legacy dicts)."""
    defaults: dict[str, object] = {"id": "P004", "name": "Áo hoodie"}
    return ProductEntityIn(**{**defaults, **kwargs})
...
async def test_reattach_keeps_active_turn_committable() -> None:
    ...
    task = asyncio.create_task(coordinator._maybe_speak(session_id, decision))
    assert await asyncio.to_thread(backend.started.wait, 1)
    revised = _product(name="Áo hoodie bản mới").to_entity()
...
async def test_pipeline_prepares_upcoming_turns_during_active_playback() -> None:
    ...
    coordinator.start(
        session_id,
        [_product(name="Áo hoodie HeyGen").to_entity()],
        activated=True,
    )
    try:
...
async def test_stop_does_not_resurrect_history_from_cancelled_preparation() -> None:
    ...
    runtime.attach(session_id, [_product().to_entity()])
    coordinator._stats[session_id] = _SessionStats()
...
async def test_tick_keeps_prepared_next_product_transition() -> None:
    ...
    products = [
        _product(id="P004", name="Áo hoodie", features=["ấm"]),
        _product(id="P001", name="Kem chống nắng", features=["nhẹ mặt"]),
    ]
    runtime.attach(
        session_id,
        [p.to_entity() for p in products],
