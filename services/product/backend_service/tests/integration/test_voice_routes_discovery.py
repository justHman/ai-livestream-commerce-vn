...
from tts.engines.base import AudioChunk, ToneEngine, TTSRequest
...
def _client() -> tuple[TestClient, EngineManager, AvatarStore]:
...
def test_failed_llm_swap_preserves_active_engine(monkeypatch) -> None:
    ...
    monkeypatch.setattr("backend.engine_manager.load_llm_engine", fail_load)
...
def test_failed_tts_swap_preserves_active_engine(monkeypatch) -> None:
    ...
    monkeypatch.setattr("backend.engine_manager.load_tts_engine", fail_load)
