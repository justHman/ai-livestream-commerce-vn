...
def test_403_on_admin(monkeypatch):
    ...
    config = AppConfig(
        render_backend="mock",
        app_env="production",
        backend_api_token="vtkn",
        admin_api_token="atkn",
        cors_origins="http://localhost",
    )
...
def test_rate_limit(monkeypatch):
    ...
    config = AppConfig(
        ...
        api_rate_limit_window_seconds=_MINUTE,
...
def test_ws_rate_limit(monkeypatch):
    ...
    config = AppConfig(
        ...
        ws_rate_limit_window_seconds=_MINUTE,
        api_rate_limit_max_keys=100,
    )
    with TestClient(_make_app(config)) as c:
...
def test_isolated(monkeypatch):
    _env(monkeypatch)
    config = AppConfig(render_backend="mock", app_env="dev")
    deps_a = _Deps(config=config)
    deps_b = _Deps(config=config)
    aa = create_app(config=config, deps=deps_a)
    ab = create_app(config=config, deps=deps_b)
    return
