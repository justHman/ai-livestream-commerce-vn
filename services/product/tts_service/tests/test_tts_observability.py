...
from tts.observability.logging.config import REDACTION_FIELD, REDACTION_MARKER
...
from tts.observability.logging.setup import reset_logging
...
def render(extra: dict[str, object]) -> str:
    ...
    handler.setFormatter(ContextFormatter(service="tts"))
    handler.addFilter(ContextFilter())
    handler.addFilter(StructuredFieldsFilter())
    logger = logging.getLogger("tts.observability.test.render")
...
def render_with_downstream_handler(extra: dict[str, object]) -> tuple[str, logging.LogRecord]:
    ...
    safe_handler.setFormatter(ContextFormatter(service="tts"))
    safe_handler.addFilter(StructuredFieldsFilter())
    downstream_handler = CaptureHandler()
    logger = logging.getLogger("tts.observability.test.downstream")
    old_handlers, old_propagate = logger.handlers[:], logger.propagate
...
def test_context_identifiers_are_exact() -> None:
    with scoped(session_id="s", request_id="r", trace_id="t", component="tts"):
        assert get_all() == {
            "session_id": "s",
            "request_id": "r",
            "trace_id": "t",
...
def test_outbound_metadata_contains_only_validated_context() -> None:
    with scoped(
        ...
        request_id="request-1",
        trace_id="trace-1",
        component="tts.client",
    ):
        assert outbound_headers() == {
            "x-session-id": "session-1",
            "x-request-id": "request-1",
            "x-trace-id": "trace-1",
            "x-component": "tts.client",
        }
...
def test_setup_is_idempotent_and_closes_owned_handler() -> None:
    with removable_temp_dir() as runtime_root:
        ...
        assert (
            sum(getattr(item, "_tts_observability_handler", False) for item in logger.handlers) == 1
        )
        reset_logging()
