...
The Director runtime, Coordinator, diagnostics, product retrieval, and coverage matching SHALL use one process-level embedding service and one embedding cache per session input identity.
...
Each active comment SHALL be routed by broad category, product target, and commerce intent before semantic clustering. Supported intents SHALL include price, promotion, stock, size/color, shipping, usage, comparison, buy intent, complaint, social, spam, and off-topic.
...
Only actionable commerce clusters SHALL enter the Director's Q&A ranking. Social comments MAY inform engagement behavior, while spam and off-topic comments SHALL NOT inflate the unanswered commerce queue.
...
The repository SHALL contain a deterministic Vietnamese commerce fixture that measures singleton ratio, expected same-intent merges, and prohibited cross-product or cross-intent merges. The effective threshold SHALL be selected from benchmark evidence rather than changed solely to reduce cluster count.
...
A Q&A window SHALL open after a product reaches Intro plus Benefit 1 completion. Within the window, clusters SHALL be ranked by the scorer (product relevance, intent actionability, size, recency, phase, new demand). Only clusters with size >= 2 SHALL be eligible. A singleton SHALL be retained in a rolling window for later merging with new demand on the same topic.
...
After a cluster is answered, its topic SHALL enter cooldown (default 120 seconds). During cooldown the same topic SHALL NOT be answered again unless new comments with distinct content arrive.
...
When a cluster for product B wins ranking during product A's stage, the system SHALL distinguish two cases:
...
The demand pivot SHALL enter when demand share >= 60%. It SHALL exit when demand share drops below 45%. The system MUST NOT re-pivot within a product that was just exited (nested pivot is forbidden). Demand for a third product C SHALL be queued.
