...
The system SHALL preserve a user-facing `display_text` representation and an exact TTS-facing `spoken_text` representation whenever normalization changes how the content will be spoken.
...
The system SHALL route a complete approved `spoken_text` through the same Change A `TextChunker` capability used for arbitrary incremental text and SHALL NOT create a script-specific chunking bypass.
...
Change B SHALL supply approved text but SHALL NOT manually construct, rewrite, or stamp `TextChunk.is_final`; normal-completion finality remains Change A's exactly-once TextChunk→AudioWindow→VideoWindow protocol.
