...
- **VieNeu-TTS-v2 weights source**: HuggingFace `pnnbao-ump/VieNeu-TTS-v2` (verified 2026-07-24: public, NOT gated, 13 files). Seeded to S3 via `upload_weights_s3.py` (HF `snapshot_download` → `aws s3 sync`). The local `.git` at `D:\Project\Code\vllm-omni` (branch `feat/vieneu-tts-v0.22`, HEAD `e3d48e0a`) is the **vllm-omni fork with VieNeu adapter CODE** (not weights); it ships in the TTS Docker image via the fork clone. S3 remains the runtime source (no HF cold pull during billable window); HF is the offline seeding source.
