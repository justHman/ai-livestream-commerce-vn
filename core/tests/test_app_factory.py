...
# Finding 1: create_app(env-driven, no deps) under RENDER_BACKEND=mock with a
# configured real LLM/TTS engine MUST load it (engine_manager.llm is not None).
# Only the cloud-specific wiring (cloud import + reconfigure_cloud) is skipped.
# Offline-test safety: with LLM_ENGINE=none/TTS_ENGINE=tone (the defaults),
# the guards skip loading → em.llm/.tts stay None → stubs still work. We test
# the STRUCTURE by registering a stub engine that ``load_engine`` will find.
...
def test_mock_mode_loads_configured_llm_when_set(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from core.llm.base import LLMEngine, LLMRequest, LLMResponse, register_engine
    ...
    # Mock backend → reconfigure_cloud was NOT called (mock doesn't use it).
    # (Indirect verification: the app booted without importing cloud wiring.)
...
def test_mock_mode_loads_configured_tts_when_set(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from core.tts.base import AudioChunk, TTSEngine, TTSRequest, register_engine
    import numpy as np
...
def test_create_app_records_llm_load_error_on_failure(
    ...
    server.py records the failure on engine_manager.llm_load_error AND
