...
sandbox (free, no credits):
  ...
  3. POST /api/v1/sessions     -> frontend-safe creds (no token/ws_url leak)
  4. POST /api/v1/sessions/{id}/say       -> reply; WS gets speak_started/ended
  5. POST /api/v1/sessions/{id}/interrupt -> WS gets interrupted
  6. POST /api/v1/sessions/{id}/stop      -> WS gets session.stopped
...
def main() -> None:
    ...
    r = c.post("/api/v1/sessions", json={"is_sandbox": True})
