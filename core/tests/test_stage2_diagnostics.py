...
from core.director.coordinator import DirectorCoordinator, _SessionStats
...
async def test_transient_turn_retries_per_turn_and_advances_only_after_success() -> None:
    ...
    runtime.attach(
        ...
        cfg=__import__("core.director.config", fromlist=["StreamConfig"]).StreamConfig(
...
async def test_tick_keeps_prepared_next_product_transition() -> None:
    ...
    session.director.state.phase = __import__(
        "core.director.state", fromlist=["Phase"]
    ).Phase.SELLING
...
async def test_completed_close_commits_real_closing_phase() -> None:
    from core.api.v1 import build_run_plan
    from core.director.config import StreamConfig
    from core.director.state import Phase
