...
from .embedder import embedder_status
from .hooks import HookPool
from .config import StreamConfig
from .scorer import rank_clusters
...
class DirectorCoordinator:
        ...
        try:
            ...
            if decision.prompt is not None and can_prepare:
                ...
                request = LLMRequest.from_prompt(
                    decision.prompt,
                    system_prompt=ds.system_prompt or None,
                )

                def generate_script(variant_index: int) -> str:
                    return "".join(
                        chunk.text
                        for chunk in self._llm.stream_chunks(
                            request,
                            session_id=session_id,
                            utterance_id=f"{decision.turn_id}:{variant_index}",
                        )
                    )
    ...
    async def _maybe_speak(self, session_id: str, decision: Decision) -> bool:
        ...
        if ds is not None:
            ...
            from ..config import BASE_SALE_PERSONA
