...
Phase A notes:
  - v3-Turbo emits 48 kHz audio; v2 emits 24 kHz. We detect the variant from
    the weights path (case-insensitive substring ``v3-turbo``).
  - Streaming (``stream``) probes ``infer_stream`` on the underlying model and
    yields incremental :class:`AudioChunk` instances; otherwise it falls back
    to a single full-waveform chunk from :meth:`synthesize`.
...
class VieNeuAdapter(TTSEngine):
    ...
    def from_config(cls, cfg: dict) -> "VieNeuAdapter":
        ...
        try:
            ...
            try:
                ...
                # Phase A: NO silent tone fallback. Re-raise so
                # engine_manager records the failure in tts_load_error and
                # /health/ready reports not-ready honestly. Only honour the
                # legacy tone fallback in opt-in dev mode.
    ...
    def synthesize(self, req: TTSRequest) -> AudioChunk:
        if self._model is None:
            if self._impl == "tone-fallback":
                ...
                # locally even if neuttsair is not installed.
                import math
    ...
    def stream(self, req: TTSRequest) -> Iterator[AudioChunk]:
        """True streaming when the underlying model exposes ``infer_stream``.

        Falls back to a single full-waveform chunk when the model lacks a
        streaming API or when streaming raises mid-flight.
        """
        if self._model is None:
            # Honour the same fallback policy as synthesize().
            yield self.synthesize(req)
            return
        infer_stream = getattr(self._model, "infer_stream", None)
        if infer_stream is None or not callable(infer_stream):
            yield self.synthesize(req)
            return
        ref = req.voice or self._default_ref
        try:
            emitted = False
            for piece in infer_stream(req.text, ref_audio=ref, ref_text=req.ref_text):
                pcm = np.asarray(piece, dtype=np.float32).reshape(-1)
                if pcm.size == 0:
                    continue
                emitted = True
