...
def _decode_mp3_to_float32(mp3_bytes: bytes, sample_rate: int) -> np.ndarray:
    ...
    with tempfile.NamedTemporaryFile(suffix=".mp3") as inf, tempfile.NamedTemporaryFile(suffix=".wav") as outf:
        inf.write(mp3_bytes)
        inf.flush()
        subprocess.run(
            ["ffmpeg", "-y", "-i", inf.name, "-ar", str(sample_rate),
             "-ac", "1", "-f", "wav", outf.name],
            check=True, capture_output=True,
        )
        import wave
        with wave.open(outf.name, "rb") as wf:
