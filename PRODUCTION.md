# PRODUCTION.md — Architecture & Portability
...
core/render/cloud.py       CloudRenderBackend  -> wraps liveavatar_api (LiveAvatar cloud) [ACTIVE]
...
`liveavatar_api/` is unchanged and now sits BEHIND the cloud adapter — it is "one service option", not
the product. `liveavatar_demo/` (mock diffusion PoC) is untouched.
...
- **Prefix caching / RadixAttention** is the big lever here: the persona + product catalog system prompt is
  ...
  RadixAttention. This is the main technical reason production uses vLLM/SGLang over llama.cpp (llama.cpp
...
- **Demo (Colab T4, low concurrency):** llama.cpp (`llama-server`, GGUF Q4_K_M) — low TTFT, low VRAM.
...
- **torch.compile:** N/A under llama.cpp; for a `transformers` path use `reduce-overhead` only with stable
...
│   ├── render/{base,cloud,self_host}.py
│   ├── config.py  store.py
│   ├── director/            # future cluster/scoring (stub)
│   └── tests/v1_smoke_test.py
├── liveavatar_api/          # LiveAvatar cloud SDK (behind cloud adapter) + examples/frontend
├── liveavatar_demo/         # mock diffusion PoC (untouched)
├── notebooks/bootstrap_colab.ipynb
└── PRODUCTION.md  PLAN.md  TASKS.md
```
...
python -m core.tests.v1_smoke_test      # free sandbox verification
uv run uvicorn core.server:app --port 8800
# Frontend: open liveavatar_api/frontend/lite.html, paste backend URL.
```

Verified 2026-06-22 on sandbox: core `/api/v1` (health→WS→start→say→interrupt→stop, no secret leak),
legacy `liveavatar_api` smoke still green, `self_host` seam returns clean NotImplemented.
